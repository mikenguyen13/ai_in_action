...
The full pipeline that this chapter formalizes can be read off a single diagram. The world supplies an unknown distribution, we observe only a finite i.i.d. sample, an algorithm searches a restricted hypothesis class for the predictor of lowest empirical (and possibly regularized) risk, and we judge the result by its true risk on fresh draws from the same distribution.

```{mermaid}
flowchart LR
    D["Unknown distribution D over X times Y"] -->|"i.i.d. sample"| S["Training set S of n examples"]
    S --> A["Learning algorithm: ERM over hypothesis class H"]
    H["Hypothesis class H (inductive bias)"] --> A
    L["Loss function l"] --> A
    A --> HH["Learned predictor h-hat"]
    HH -->|"evaluate on fresh draws"| R["True risk R(h-hat)"]
    D -.->|"defines"| R
```
...
To see why the conditional mean is optimal under squared loss, write any predictor's risk as $\mathbb{E}[(h(x) - y)^2]$ and condition on $x$. For a fixed $x$, let $m(x) = \mathbb{E}[y \mid x]$. Then
$$
\mathbb{E}\big[(h(x) - y)^2 \mid x\big] = \big(h(x) - m(x)\big)^2 + \mathbb{E}\big[(y - m(x))^2 \mid x\big],
$$
because the cross term $2(h(x) - m(x))\,\mathbb{E}[y - m(x) \mid x]$ vanishes by the definition of $m(x)$. The first piece is nonnegative and is zeroed exactly by the choice $h(x) = m(x)$; the second piece does not depend on $h$ at all. The second term, averaged over $x$, is precisely the irreducible Bayes error $\sigma^2$ that reappears in the bias variance decomposition of Section 4.3.
...
where $\hat{p}$ is a predicted probability vector, is the workhorse for both logistic regression and modern neural classifiers. It is not an arbitrary choice: minimizing average cross entropy is exactly maximizing the likelihood of the labels under the model's predicted probabilities, so it inherits the asymptotic guarantees of maximum likelihood estimation. The hinge loss $\max(0, 1 - y \cdot \hat{y})$ underlies support vector machines and induces a margin, penalizing predictions that are correct but not confident. The exponential loss $\exp(-y \cdot \hat{y})$ drives AdaBoost. Each is a surrogate chosen because it is convex and (Fisher) consistent, meaning that the predictor minimizing the surrogate risk over all functions agrees in sign with the Bayes classifier, so minimizing it drives the predictor toward Bayes optimal as data grows.
...
The richness of a hypothesis class is its capacity. For binary classification the classical measure is the Vapnik Chervonenkis (VC) dimension. A set of points is shattered by $\mathcal{H}$ if, for every one of the $2^m$ possible labelings of $m$ points, some $h \in \mathcal{H}$ realizes that labeling. The VC dimension $\mathrm{VC}(\mathcal{H})$ is the size of the largest set that $\mathcal{H}$ can shatter.

As a concrete example, consider thresholded linear classifiers in the plane, $h(x) = \mathrm{sign}(w^\top x + b)$ with $x \in \mathbb{R}^2$. Any three points in general position (not collinear) can be split in all $2^3 = 8$ ways by some line, so three points are shattered. But no set of four points can be shattered: by Radon's theorem any four points in the plane can be partitioned into two subsets whose convex hulls intersect, and that particular labeling (one subset positive, the other negative) cannot be separated by any straight line. Hence $\mathrm{VC} = 3$, matching the general formula $d + 1$ for linear classifiers in $\mathbb{R}^d$.

Higher capacity means more expressive power but also a greater risk of fitting noise. Related measures include Rademacher complexity, which captures how well a class can correlate with random $\pm 1$ labels, and covering numbers. These are not just theoretical ornaments; they appear directly in the bounds that connect training error to test error.

### 4.3 The bias variance decomposition

Capacity controls a fundamental tradeoff. Treat the learned predictor $\hat{h}$ as random because it depends on the random training sample $S$. For a fixed test point $x$, write $\bar{h}(x) = \mathbb{E}_S[\hat{h}(x)]$ for the average prediction across resamples. The expected squared error decomposes as

$$
\mathbb{E}_{S, y}\big[(\hat{h}(x) - y)^2\big] = \underbrace{\big(\bar{h}(x) - m(x)\big)^2}_{\text{Bias}^2,\ \text{too rigid}} + \underbrace{\mathbb{E}_S\big[(\hat{h}(x) - \bar{h}(x))^2\big]}_{\text{Variance},\ \text{too sensitive}} + \underbrace{\sigma^2}_{\text{irreducible}},
$$

where $m(x) = \mathbb{E}[y \mid x]$ is the Bayes regressor from Section 1.2 and $\sigma^2$ is its irreducible noise. The bias term measures how far the class is systematically displaced from the truth; the variance term measures how much the fit jitters as the sample changes. A low capacity class has high bias: it cannot represent the truth and underfits. A high capacity class has high variance: it chases noise in the particular training sample and overfits. The classical prescription is to tune capacity to the point where the sum is minimized, the sweet spot of the U shaped test error curve. Section 6 revisits how deep learning complicates this picture without overturning it.

### 4.4 A worked example: fitting a polynomial

Make the tradeoff concrete. Suppose the truth is $m(x) = \sin(2\pi x)$ on $[0, 1]$ and we observe $n$ noisy points $y_i = \sin(2\pi x_i) + \varepsilon_i$ with $\varepsilon_i \sim \mathcal{N}(0, \sigma^2)$. Our hypothesis class is polynomials of degree $d$, fit by least squares, so $d$ is the capacity knob.

- Degree $0$ or $1$. A constant or a line cannot bend to follow a full sine wave. Every resample produces nearly the same wrong curve, so variance is tiny but bias is large. This is underfitting.
- Degree $3$ to $5$. A cubic or quintic tracks one period of the sine closely. Bias has collapsed and variance is still modest, so the total error is near its minimum. This is the sweet spot.
- Degree $15$ with $n = 20$. The polynomial has nearly as many free coefficients as data points. It threads every noisy observation, oscillating wildly between them. Bias is essentially zero, but a single resampled point swings the curve dramatically, so variance explodes. This is overfitting.

The training error falls monotonically as $d$ rises, because a higher degree class contains every lower degree fit and can only reduce empirical risk. The test error instead traces the U shape: it falls while reducing bias dominates, then climbs once growing variance takes over. Cross validation (Section 5.3) is how we locate the bottom of that U without peeking at the test set. Adding an $L_2$ penalty on the coefficients (Section 5.2) lets us keep a high degree $d$ for expressiveness while $\lambda$ reins in the variance, which is often more effective than discretely choosing $d$.
...
Making the capacity term concrete, a standard VC bound for binary classification with the zero one loss states that with probability at least $1 - \delta$, for all $h \in \mathcal{H}$,

$$
R(h) \leq \hat{R}_S(h) + \sqrt{\frac{8\big(\mathrm{VC}(\mathcal{H})\,\log\frac{2en}{\mathrm{VC}(\mathcal{H})} + \log\frac{4}{\delta}\big)}{n}}.
$$

The qualitative shape is exactly the inequality above: the numerator grows roughly linearly in the VC dimension and the denominator is the sample size, so the gap behaves like $\sqrt{\mathrm{VC}(\mathcal{H}) / n}$ up to logarithmic factors. A useful rule of thumb that falls out of such bounds is that the sample size needed for reliable generalization scales with the capacity of the class: you want $n$ comfortably larger than $\mathrm{VC}(\mathcal{H})$.
...
The paradigm is most useful when its failure modes are kept in view.

- Breaking the i.i.d. assumption silently. Time series, grouped data (multiple rows per patient or user), and any sample selected non-randomly violate the foundation of every bound here. Validation folds that split such data at random leak information and report optimistic error. Split by time or by group instead.
- Test set leakage through repeated peeking. Each time the test set guides a decision it stops being a fair estimate of true risk. Selecting among many models on the same test set inflates capacity usage exactly as enlarging $\mathcal{H}$ does.
- Preprocessing fit on the full data. Computing scaling, imputation, or feature selection on all rows before splitting leaks test statistics into training. Fit every transform inside the training fold only.
- Optimizing the wrong loss. A model tuned on accuracy can be useless when classes are imbalanced or when false positives and false negatives carry different costs. Choose the loss and the evaluation metric to match the real decision, as Section 3.4 urges.
- Reading parameter count as capacity. As Section 6.2 notes, for overparameterized models the number of weights is a poor proxy for effective capacity. Judge generalization by held out performance, not by model size.

### 6.5 The enduring discipline
...
10. Bartlett, P. L., Jordan, M. I., and McAuliffe, J. D. "Convexity, Classification, and Risk Bounds." Journal of the American Statistical Association, 2006. https://doi.org/10.1198/016214505000000907
11. Vapnik, V. N. and Chervonenkis, A. Ya. "On the Uniform Convergence of Relative Frequencies of Events to Their Probabilities." Theory of Probability and Its Applications, 1971. https://doi.org/10.1137/1116025
