...
```python
# Conceptual, not executable: prefer a stable solver over explicit inverse
import numpy as np

# w_star = np.linalg.inv(X.T @ X) @ X.T @ y   # fragile
w_star, *_ = np.linalg.lstsq(X, y, rcond=None)  # QR or SVD based, stable
```
...
```python
# Conceptual batch gradient descent for least squares
