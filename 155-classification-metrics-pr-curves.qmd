...
The companion library `aiinaction` ships a small, dependency-light implementation of the rank based machinery developed above: `pr_curve` builds the operating points over the distinct score thresholds, `average_precision` computes the rank based AP of Section 3.1, and `auprc_trapezoid` integrates the resulting curve by the trapezoidal rule. The three language bindings agree to floating point tolerance on a shared fixture, and the rank based AP matches scikit-learn's `average_precision_score` exactly.

The worked example below uses eight instances whose descending score order interleaves positives and negatives, producing the sawtooth precision behaviour discussed in Section 2.2. The rank based AP is $0.7708\overline{3}$, while the trapezoidal area over the same realized points is the smaller $0.48125$, a concrete reminder that the estimator must be named when a number is reported.
...
```{python}
from aiinaction.ch155_pr_curves import pr_curve, average_precision, auprc_trapezoid
...
for tau, r, p in zip(curve.thresholds, curve.recall, curve.precision):
    print(f"{tau:9.2f}  {r:6.3f}  {p:9.4f}")

print(f"\nAverage precision (rank-based): {average_precision(y_true, scores):.6f}")
print(f"AUPRC (trapezoidal):            {auprc_trapezoid(y_true, scores):.6f}")
```
...
for (tau, r, p) in zip(curve.thresholds, curve.recall, curve.precision)
    println(rpad(round(tau; digits=2), 11), rpad(round(r; digits=3), 8),
            round(p; digits=4))
end

println("\nAverage precision (rank-based): ", round(average_precision(y_true, scores); digits=6))
println("AUPRC (trapezoidal):            ", round(auprc_trapezoid(y_true, scores); digits=6))
```

## Rust

```rust
use aiinaction::ch155_pr_curves::{pr_curve, average_precision, auprc_trapezoid};

fn main() {
    let y_true = [1, 0, 1, 1, 0, 1, 0, 0];
    let scores = [0.9, 0.8, 0.7, 0.6, 0.55, 0.54, 0.53, 0.51];

    let curve = pr_curve(&y_true, &scores).unwrap();
    println!("threshold  recall  precision");
    for k in 0..curve.len() {
        println!(
            "{:9.2}  {:6.3}  {:9.4}",
            curve.thresholds[k], curve.recall[k], curve.precision[k]
        );
    }

    println!(
        "\nAverage precision (rank-based): {:.6}",
        average_precision(&y_true, &scores).unwrap()
    );
    println!(
        "AUPRC (trapezoidal):            {:.6}",
        auprc_trapezoid(&y_true, &scores).unwrap()
    );
}
```
...
## 7. Practical Reporting Guidance
