...
The companion library ships a small, dependency-light implementation of all three metrics in Python, Julia, and Rust under the module `ch162_ranking_metrics`. Each function takes a per-query relevance list in ranked order, treats a strictly positive entry as relevant, and supports an optional top-$k$ cutoff. The three implementations are checked against a shared set of numeric fixtures so they agree to floating-point tolerance.

The derivations above translate directly to code. Reciprocal rank scans for the first positive entry and returns $1/\mathrm{rank}$. Average precision accumulates $\mathrm{hits}/k$ at every hit position and divides by the number of relevant items $R$. DCG sums $(2^{\mathrm{rel}} - 1)/\log_2(j+1)$ over positions, and NDCG normalizes by the DCG of the descending-sorted grades.
...
```{python}
from aiinaction.ch162_ranking_metrics import (
...
# MRR: three known-item queries, the single relevant item at ranks 1, 3, 2.
mrr_queries = [[1, 0, 0], [0, 0, 1], [0, 1, 0]]
print(f"MRR = {mean_reciprocal_rank(mrr_queries):.4f}")

# MAP: two queries under binary relevance.
ap_q1 = [1, 0, 1, 0, 0, 1]
ap_q2 = [0, 1, 1, 0]
print(f"AP(q1) = {average_precision(ap_q1):.4f}")
print(f"MAP    = {mean_average_precision([ap_q1, ap_q2]):.4f}")

# NDCG: graded relevance, exponential gain with logarithmic discount.
grades = [3, 2, 0, 1, 2]
print(f"DCG@5  = {dcg(grades):.4f}")
print(f"NDCG@5 = {ndcg(grades):.4f}")
```
...
# MRR: three known-item queries, the single relevant item at ranks 1, 3, 2.
mrr_queries = [[1, 0, 0], [0, 0, 1], [0, 1, 0]]
println("MRR = ", round(mean_reciprocal_rank(mrr_queries); digits=4))

# MAP: two queries under binary relevance.
ap_q1 = [1, 0, 1, 0, 0, 1]
ap_q2 = [0, 1, 1, 0]
println("AP(q1) = ", round(average_precision(ap_q1); digits=4))
println("MAP    = ", round(mean_average_precision([ap_q1, ap_q2]); digits=4))

# NDCG: graded relevance, exponential gain with logarithmic discount.
grades = [3, 2, 0, 1, 2]
println("DCG@5  = ", round(dcg(grades); digits=4))
println("NDCG@5 = ", round(ndcg(grades); digits=4))
```

## Rust

```rust
use aiinaction::ch162_ranking_metrics::{
    average_precision, dcg, mean_average_precision, mean_reciprocal_rank, ndcg,
};

fn main() {
    // MRR: three known-item queries, the relevant item at ranks 1, 3, 2.
    let mrr_queries = vec![
        vec![1.0, 0.0, 0.0],
        vec![0.0, 0.0, 1.0],
        vec![0.0, 1.0, 0.0],
    ];
    println!("MRR = {:.4}", mean_reciprocal_rank(&mrr_queries, None).unwrap());

    // MAP: two queries under binary relevance.
    let ap_q1 = vec![1.0, 0.0, 1.0, 0.0, 0.0, 1.0];
    let ap_q2 = vec![0.0, 1.0, 1.0, 0.0];
    println!("AP(q1) = {:.4}", average_precision(&ap_q1, None, None).unwrap());
    let queries = vec![ap_q1.clone(), ap_q2];
    println!("MAP    = {:.4}", mean_average_precision(&queries, None, None).unwrap());

    // NDCG: graded relevance, exponential gain with logarithmic discount.
    let grades = vec![3.0, 2.0, 0.0, 1.0, 2.0];
    println!("DCG@5  = {:.4}", dcg(&grades, None, true).unwrap());
    println!("NDCG@5 = {:.4}", ndcg(&grades, None, true).unwrap());
}
```
