...
1.  **Completeness.** If a valid assignment exists, the resolver finds one; if none exists, it reports a genuine conflict rather than installing a broken set. This is the property `pip`'s historical resolver lacked.

2.  **Determinism.** Given the same inputs (the same `pyproject.toml`, the same index state), resolution returns the same assignment. Determinism is what makes the lock file meaningful: it is the serialized $\sigma$.

3.  **Best-effort optimality.** Among valid assignments the resolver prefers the newest versions consistent with your constraints, so you get bug fixes "for free" without manual pinning.
...
### Worked example: an unsatisfiable diamond
...
### What the lock file does and does not guarantee
