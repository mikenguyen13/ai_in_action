...
The flow of work in this chapter runs in one direction, from an underspecified human goal to a procedure whose guarantees and costs are understood.

```{mermaid}
flowchart LR
    A["Vague real world goal"] --> B["Well posed problem: spaces, target, loss"]
    B --> C["Representation: features or learned embeddings"]
    C --> D["Hypothesis class and inductive bias"]
    D --> E["Algorithm: learning and inference"]
    E --> F["Bounded solution with known cost and error"]
    F -. "feedback and reframing" .-> A
```

The dashed arrow matters as much as the solid ones. Framing is rarely correct on the first pass, and the discipline is iterative: deployment surfaces a leak, a distribution shift, or an objective mismatch that sends the practitioner back to redefine the problem rather than to retune the model.
...
A useful test for a good decomposition is whether the interfaces between parts are narrow and stable. If two subproblems must constantly exchange large amounts of state, they were probably one problem. This is the same principle of low coupling and high cohesion that governs modular software design, and it has a precise payoff in machine learning: components joined by a narrow interface can be evaluated, retrained, and replaced independently, whereas tightly entangled components fail together and resist diagnosis.

There is a subtle hazard unique to learned systems. When a decomposition feeds the output of one learned component into the input of another, the consuming component begins to depend on the idiosyncrasies of its upstream supplier, a coupling that Sculley and colleagues named an entanglement antipattern, summarized as the maxim that changing anything changes everything. A decomposition that looks clean on a whiteboard can still be tightly coupled in practice if the interface carries a learned, drifting signal rather than a stable contract. Good decomposition in AI therefore demands that interfaces be not only narrow but also versioned and monitored.
...
First, the input space, often written as a set of objects $x$ drawn from a domain $\mathcal{X}$. Second, the output space $\mathcal{Y}$, the set of permissible answers. Third, an unknown target relationship, usually framed as a distribution $\mathcal{D}$ over pairs $(x, y)$, that we wish to approximate. Fourth, a hypothesis class $\mathcal{H}$, the family of candidate functions we are willing to consider. Fifth, a loss function $\ell(\hat{y}, y)$ that quantifies the cost of predicting $\hat{y}$ when the truth is $y$.

We can now state the object of learning precisely. For a hypothesis $h$, define its **risk** (true error) as the expected loss

$$
R(h) \;=\; \mathbb{E}_{(x,y)\sim\mathcal{D}}\big[\ell(h(x), y)\big].
$$

Because $\mathcal{D}$ is unknown, we cannot evaluate $R(h)$ directly. Instead we observe a finite training sample $S = \{(x_i, y_i)\}_{i=1}^{n}$ drawn independently from $\mathcal{D}$ and compute the **empirical risk**

$$
\hat{R}_S(h) \;=\; \frac{1}{n}\sum_{i=1}^{n} \ell(h(x_i), y_i).
$$

The well-posed problem is to find $h \in \mathcal{H}$ with small true risk $R(h)$, given only the ability to measure $\hat{R}_S(h)$. The standard strategy, **empirical risk minimization**, returns the hypothesis $\hat{h}_S = \arg\min_{h\in\mathcal{H}} \hat{R}_S(h)$. The entire theory of generalization is the study of when minimizing the proxy $\hat{R}_S$ also makes the target $R$ small.

::: {.callout-note}
## Definition: a well-posed learning problem

A learning problem is well-posed once the tuple $(\mathcal{X}, \mathcal{Y}, \mathcal{D}, \mathcal{H}, \ell)$ is fully specified together with the sampling protocol that produces $S$. Every element is a commitment. Leaving any one of them implicit means the team is, in effect, optimizing an objective no one has written down.
:::
...
The second is sample complexity: how much data is required to learn a hypothesis of a given accuracy with a given confidence. Statistical learning theory, through tools such as the Vapnik-Chervonenkis (VC) dimension, relates the richness of a hypothesis class to the number of examples needed to avoid overfitting. The VC dimension $\mathrm{VC}(\mathcal{H})$ is the size of the largest set of points that $\mathcal{H}$ can label in all possible ways, a combinatorial measure of how expressive the class is. A central result of the theory makes the trade-off quantitative: with probability at least $1 - \delta$ over the draw of a sample of size $n$, every hypothesis $h \in \mathcal{H}$ satisfies a generalization bound of the form

$$
R(h) \;\le\; \hat{R}_S(h) \;+\; O\!\left(\sqrt{\frac{\mathrm{VC}(\mathcal{H})\,\log n \;+\; \log(1/\delta)}{n}}\right).
$$

The structure of this inequality is the entire lesson, even setting the constants aside. The true risk we care about is bounded by the empirical risk we can measure, plus a penalty that grows with the capacity $\mathrm{VC}(\mathcal{H})$ of the hypothesis class and shrinks as the sample size $n$ grows. A richer class can drive the first term down but inflates the second, and only more data can pay that inflation back. This is the formal heart of the bias-variance trade-off, and it explains why throwing a billion-parameter model at a thousand examples is usually folly: the empirical risk can be driven to zero while the bound on true risk stays vacuous. The same inequality also explains why useful inductive biases, discussed below, are not a luxury. By shrinking the effective capacity of $\mathcal{H}$ to functions that respect the structure of the problem, they tighten the generalization gap without requiring more labels.
...
A small numerical illustration makes the choice of metric concrete. Suppose a query returns five documents whose graded relevances, in the order the system presents them, are $(3, 2, 3, 0, 1)$ on a scale where larger is better. Discounted cumulative gain weights each relevance by a logarithmic position discount,

$$
\mathrm{DCG} \;=\; \sum_{i=1}^{5} \frac{2^{\mathrm{rel}_i} - 1}{\log_2(i + 1)}.
$$

For this ranking the gains are $\tfrac{7}{1.000}, \tfrac{3}{1.585}, \tfrac{7}{2.000}, \tfrac{0}{2.322}, \tfrac{1}{2.585}$, summing to roughly $12.78$. The ideal ranking sorts the same relevances as $(3, 3, 2, 1, 0)$, giving an ideal DCG of about $13.71$, so the normalized score is $\mathrm{nDCG} = 12.78 / 13.71 \approx 0.93$. The exponential numerator $2^{\mathrm{rel}_i} - 1$ rewards placing a highly relevant document near the top far more than a marginally relevant one, and the logarithmic discount encodes the empirical fact that users attend to early positions. A plain accuracy metric, which treats all positions alike, would be blind to exactly the structure that makes a ranking good. Choosing nDCG over accuracy is therefore not a tuning detail; it is part of posing the problem correctly.
...
### 7.1 When to Lean on These Habits, and How They Fail

Computational thinking pays the largest dividends at the start of a project, when the problem is still soft and a single framing decision will echo through everything built afterward. It is most valuable when the mandate arrives as a business sentence rather than a dataset, when the cost of errors is asymmetric or hard to name, when labels are scarce or themselves suspect, and when the system must run inside a real latency, memory, or fairness budget. In each of these situations the deciding move is made before any model is chosen, and a deliberate framing pass is the highest-leverage hour the team will spend.

The discipline also has characteristic failure modes worth naming. The first is analysis paralysis: framing can become an excuse never to build, when in truth a quick baseline often teaches more about the problem than another round of whiteboard abstraction. The second is premature or false abstraction, forcing a problem into a familiar class (almost always supervised classification) because the toolkit is comfortable, when the true structure is sequential, unsupervised, or causal. The third is optimizing a proxy whose gap to the real objective is never revisited, so the team faithfully improves an offline metric while the online measure stagnates or declines. The fourth is treating the formulation as fixed once written, when the honest practice is to revisit $(\mathcal{X}, \mathcal{Y}, \mathcal{D}, \mathcal{H}, \ell)$ whenever the data, the costs, or the deployment context shift. The remedy for all four is the same: hold framing and building in tension, let each correct the other, and treat the problem statement as a living artifact rather than a one-time gate.
