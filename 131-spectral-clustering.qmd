...
1. Compute the degree matrix $D$ and the symmetric normalized Laplacian $L_{\text{sym}} = I - D^{-1/2} W D^{-1/2}$.
2. Find the $k$ smallest eigenvectors of $L_{\text{sym}}$ and stack them as the columns of $U$.
3. Form $T$ by normalizing each row of $U$ to unit length: $T_{ij} = U_{ij} / \sqrt{\sum_j U_{ij}^2}$.
...
The companion `aiinaction` packages ship a small, dependency-light reference implementation of the Ng-Jordan-Weiss algorithm, built entirely from scratch: an RBF affinity matrix, the symmetric normalized Laplacian, a cyclic Jacobi eigensolver for the spectrum, the row-normalized embedding, and a deterministic Lloyd k-means. Determinism, in particular a fixed furthest-point seeding for k-means and a sign convention on the eigenvectors, is what lets the Python, Julia, and Rust ports agree on the same labels to within `1e-9`. The same code is installable from each language's package directory in this repository.

The example below clusters two well-separated blobs in the plane. Because the inter-cluster affinities are essentially zero, the second smallest eigenvalue of the Laplacian is tiny and the spectral gap is wide, so the embedding splits the six points cleanly into two groups.
...
```{python}
from aiinaction.ch131_spectral_clustering import (
    rbf_affinity,
...
X = [
    ...
    [5.0, 5.0], [5.2, 4.9], [4.9, 5.1],    # blob B
]

W = rbf_affinity(X, sigma=1.0)
L = normalized_laplacian(W)
eigvals, _ = jacobi_eigh(L)

labels = spectral_clustering(X, k=2, sigma=1.0)

print("two smallest Laplacian eigenvalues:",
      [round(float(v), 6) for v in eigvals[:2]])
print("spectral gap lambda_3 - lambda_2:", round(float(eigvals[2] - eigvals[1]), 4))
...
W = rbf_affinity(X, 1.0)
L = normalized_laplacian(W)
eigvals, _ = jacobi_eigh(L)

labels = spectral_clustering(X, 2, 1.0)

println("two smallest eigenvalues: ", round.(eigvals[1:2], digits=6))
println("cluster labels (0-based): ", labels)
# => labels == [0, 0, 0, 1, 1, 1]
```
...
```rust
use aiinaction::ch131_spectral_clustering::{
    jacobi_eigh, normalized_laplacian, rbf_affinity, spectral_clustering,
};
...
fn main() {
    ...
    let w = rbf_affinity(&x, 1.0).unwrap();
    let l = normalized_laplacian(&w).unwrap();
    let (eigvals, _) = jacobi_eigh(&l).unwrap();

    let labels = spectral_clustering(&x, 2, 1.0).unwrap();

    println!("two smallest eigenvalues: {:?}", &eigvals[..2]);
...
All three implementations expose the same small API: `rbf_affinity`, `normalized_laplacian`, `jacobi_eigh`, `spectral_embedding`, `kmeans`, and the end-to-end `spectral_clustering`. They share the exact fixtures used in the cross-language parity tests, so the labels and eigenvalues above are identical in every language.
