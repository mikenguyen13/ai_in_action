...
```python
# Conceptual prediction from a trained linear SVM.
# alpha, y, X are arrays over the support vectors only.
def decision_function(x, alpha, y, X_sv, b):
    score = sum(a * yi * (xi @ x) for a, yi, xi in zip(alpha, y, X_sv))
    return score + b
# For the linear case, precompute w = sum_i alpha_i y_i x_i once
# and use w @ x + b, which is far faster at inference time.
```
...
```python
# Pegasos-style stochastic subgradient step (sketch).
# lam is the regularization strength; t is the iteration count.
