...
### 4.1 Runnable DIANA on the worked example

The three tabs below run the exact $\{1, 2, 3, 20, 21, 22\}$ example through a from-scratch DIANA implementation shipped with the book. Scikit-learn provides agglomerative clustering but no divisive variant; DIANA lives in R via `cluster::diana`. The `aiinaction` packages give you a small, tested, tri-language reference instead. Install the Python package with `pip install -e .` from the repository root (the Julia package is `AIInAction`, the Rust crate is `aiinaction`).

The API is deliberately small. `macnaughton_smith_split(dist, members)` performs one splinter split, `diana(dist)` runs the full top-down loop and returns the ordered list of splits (largest diameter first), and `diana_labels(dist, k)` cuts the hierarchy into `k` flat clusters. All three operate on a precomputed dissimilarity matrix, so any metric you like can feed them.
...
pts = [1.0, 2.0, 3.0, 20.0, 21.0, 22.0]
n = len(pts)
dist = [[abs(pts[i] - pts[j]) for j in range(n)] for i in range(n)]

splinter, remainder = macnaughton_smith_split(dist, list(range(n)))
print("first split splinter:", splinter)
print("first split remainder:", remainder)

print("\nfull dendrogram (largest diameter first):")
for s in diana(dist):
    print(f"  diam={s.diameter:5.1f}  {list(s.parent)} -> "
          f"{list(s.splinter)} | {list(s.remainder)}")

print("\nflat labels at k=2:", diana_labels(dist, 2))
print("flat labels at k=3:", diana_labels(dist, 3))
```

The first cut is the obvious macro boundary, $\{0, 1, 2\}$ against $\{3, 4, 5\}$ (the indices of the two tight triples), recovered in a single pass with no enumeration of bipartitions. The diameters printed in the dendrogram decrease monotonically, the readable-tree property the largest-diameter selection rule is designed to produce.
...
pts  = [1.0, 2.0, 3.0, 20.0, 21.0, 22.0]
n    = length(pts)
dist = [abs(pts[i] - pts[j]) for i in 1:n, j in 1:n]

# Member indices are 1-based in Julia, so the macro cut is {1,2,3} | {4,5,6}.
splinter, remainder = macnaughton_smith_split(dist, collect(1:n))
println("splinter: ", splinter, "  remainder: ", remainder)

for s in diana(dist)
    println("diam=", s.diameter, "  ", s.parent, " -> ",
            s.splinter, " | ", s.remainder)
end

println("labels k=2: ", diana_labels(dist, 2))   # 0-based labels: [0,0,0,1,1,1]
println("labels k=3: ", diana_labels(dist, 3))   # [0,1,1,2,2,2]
```

## Rust

```rust
use aiinaction::ch126_diana::{diana, diana_labels, macnaughton_smith_split};
...
fn main() {
    ...
    let (splinter, remainder) =
        macnaughton_smith_split(&dist, &[0, 1, 2, 3, 4, 5]).unwrap();
    println!("splinter: {:?}  remainder: {:?}", splinter, remainder);

    for s in diana(&dist).unwrap() {
        println!(
            "diam={:.1}  {:?} -> {:?} | {:?}",
            s.diameter, s.parent, s.splinter, s.remainder
        );
    }

    println!("labels k=2: {:?}", diana_labels(&dist, 2).unwrap()); // [0,0,0,1,1,1]
    println!("labels k=3: {:?}", diana_labels(&dist, 3).unwrap()); // [0,1,1,2,2,2]
}
```

:::

All three implementations agree on the shared fixture by construction: the cross-language parity tests assert the same splits, diameters, and flat labels to within `1e-9`.
