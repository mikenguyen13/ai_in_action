...
The `aiinaction` package ships the external indices of this chapter as small,
validated, dependency-light functions that agree numerically across Python, Julia,
and Rust. The information-theoretic and pair-counting metrics (mutual information,
normalized mutual information, homogeneity, completeness, V-measure, and
Fowlkes-Mallows) live in the `ch164_clustering_metrics` module; the silhouette
coefficient and the adjusted Rand index already live in
`ch132_clustering_validation` and are re-exported rather than reimplemented, so a
single import gives the full toolkit.

All three implementations compute mutual information directly from the contingency
table $n_{ij} = |C_i \cap P_j|$,

$$
\mathrm{MI}(U, V) = \sum_{i, j} \frac{n_{ij}}{n} \log \frac{n\, n_{ij}}{a_i\, b_j},
$$

summing only over nonzero cells, then derive homogeneity as $\mathrm{MI}/H(V)$ and
completeness as $\mathrm{MI}/H(U)$ via the identity $H(V \mid U) = H(V) - \mathrm{MI}$.
The example below scores a clustering with three true classes against a prediction
that splits one class and merges nothing, so homogeneity and completeness diverge.
...
```{python}
from aiinaction.ch164_clustering_metrics import (
    contingency_matrix,
    mutual_information,
    ...
    adjusted_rand_index,  # re-exported from ch132
)

# 3 true classes; the prediction splits class 0 and class 2 into finer clusters.
labels_true = [0, 0, 0, 0, 1, 1, 1, 2, 2, 2]
labels_pred = [0, 0, 1, 1, 1, 2, 2, 3, 3, 3]

print("contingency table:")
print(contingency_matrix(labels_true, labels_pred))
print(f"mutual information : {mutual_information(labels_true, labels_pred):.6f} nats")
print(f"NMI (arithmetic)   : {normalized_mutual_information(labels_true, labels_pred):.6f}")
print(f"homogeneity        : {homogeneity(labels_true, labels_pred):.6f}")
print(f"completeness       : {completeness(labels_true, labels_pred):.6f}")
print(f"V-measure          : {v_measure(labels_true, labels_pred):.6f}")
print(f"Fowlkes-Mallows    : {fowlkes_mallows_index(labels_true, labels_pred):.6f}")
print(f"adjusted Rand index: {adjusted_rand_index(labels_true, labels_pred):.6f}")
```

Homogeneity exceeds completeness here: the prediction never mixes two true classes
inside one cluster (so clusters are pure), but it shatters classes 0 and 2 across
several clusters (so classes are not intact). That asymmetry is exactly the
diagnostic the V-measure decomposition is built to expose.

## Julia

```julia
using AIInAction.Ch164ClusteringMetrics
using AIInAction.Ch132ClusteringValidation: adjusted_rand_index

labels_true = [0, 0, 0, 0, 1, 1, 1, 2, 2, 2]
labels_pred = [0, 0, 1, 1, 1, 2, 2, 3, 3, 3]

println("contingency table:")
display(contingency_matrix(labels_true, labels_pred))
println("\nmutual information : ", round(mutual_information(labels_true, labels_pred); digits=6), " nats")
println("NMI (arithmetic)   : ", round(normalized_mutual_information(labels_true, labels_pred); digits=6))
println("homogeneity        : ", round(homogeneity(labels_true, labels_pred); digits=6))
println("completeness       : ", round(completeness(labels_true, labels_pred); digits=6))
println("V-measure          : ", round(v_measure(labels_true, labels_pred); digits=6))
println("Fowlkes-Mallows    : ", round(fowlkes_mallows_index(labels_true, labels_pred); digits=6))
println("adjusted Rand index: ", round(adjusted_rand_index(labels_true, labels_pred); digits=6))
```

## Rust

```rust
use aiinaction::ch164_clustering_metrics::{
    contingency_matrix, mutual_information, normalized_mutual_information,
    homogeneity, completeness, v_measure, fowlkes_mallows_index, AverageMethod,
};
use aiinaction::ch132_clustering_validation::adjusted_rand_index;

fn main() {
    let labels_true = [0i64, 0, 0, 0, 1, 1, 1, 2, 2, 2];
    let labels_pred = [0i64, 0, 1, 1, 1, 2, 2, 3, 3, 3];

    let (table, _, _) = contingency_matrix(&labels_true, &labels_pred).unwrap();
    println!("contingency table: {:?}", table);
    println!("mutual information : {:.6} nats", mutual_information(&labels_true, &labels_pred).unwrap());
    println!("NMI (arithmetic)   : {:.6}",
        normalized_mutual_information(&labels_true, &labels_pred, AverageMethod::Arithmetic).unwrap());
    println!("homogeneity        : {:.6}", homogeneity(&labels_true, &labels_pred).unwrap());
    println!("completeness       : {:.6}", completeness(&labels_true, &labels_pred).unwrap());
    println!("V-measure          : {:.6}", v_measure(&labels_true, &labels_pred, 1.0).unwrap());
    println!("Fowlkes-Mallows    : {:.6}", fowlkes_mallows_index(&labels_true, &labels_pred).unwrap());
    println!("adjusted Rand index: {:.6}", adjusted_rand_index(&labels_true, &labels_pred).unwrap());
}
```
