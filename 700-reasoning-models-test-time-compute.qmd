...
The shift matters enough that a treatment of LLMs that stops at instruction-tuned chat models is a generation behind. This chapter develops what reasoning models are, the mathematics that explains why spending compute at inference helps, the two roads to building them (OpenAI's o-series and DeepSeek's open R1), the test-time compute methods that operationalize thinking, the economics of a thinking budget, and the honest limits. It then turns to **production code**: runnable demonstrations of self-consistency majority voting and best-of-N selection that reproduce the compute-versus-accuracy tradeoff on a laptop, plus show-only invocations for the GPU-scale methods (tree search, process reward models, large reasoning models) that a reader can run on appropriate hardware.
...
This section is **executed on CPU**. Every number printed below is computed at render time by the code shown, on small deterministic examples that finish in seconds. The two core parallel methods, self-consistency and best-of-N, are implemented from first principles so the compute-versus-accuracy tradeoff of section @sec-700-math is reproduced rather than asserted. A short block then exercises a real (tiny) Hugging Face model to show the mechanics and per-sample latency of parallel sampling, followed by show-only invocations for the GPU-scale methods.
