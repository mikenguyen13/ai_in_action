...
```{mermaid}
%%| label: fig-pipeline
%%| fig-cap: "The four stages of a scraping pipeline and the failure path that keeps a single bad page from aborting a large run."
flowchart LR
...
::: {.callout-tip title="The token bucket, precisely"}
...
```{mermaid}
%%| label: fig-decision
%%| fig-cap: "Choosing the cheapest acquisition method that works. Each downward step costs more in time, memory, and fragility."
flowchart TD
...
### 6.5 When to Scrape, and Common Pitfalls
...
11. Brooker, M. "Exponential Backoff and Jitter." Amazon Web Services Architecture Blog, 2015. https://aws.amazon.com/blogs/architecture/exponential-backoff-and-jitter/
