...
The book ships a small, dependency-light implementation of EM for Gaussian mixtures in all three languages of the companion libraries: the Python package `aiinaction`, the Julia package `AIInAction`, and the Rust crate `aiinaction`. The module is `ch130_gmm_em` and exposes the same five-function API in each language: `gaussian_pdf` for the component density, `e_step` for the responsibilities, `m_step` for the weighted re-estimation, `log_likelihood` for the incomplete-data objective, and `fit_gmm` to run the full alternation to convergence. To keep the three implementations bit-for-bit comparable under a shared test suite, `fit_gmm` takes explicit initial parameters rather than seeding randomly, so the same data and initialization produce the same trajectory everywhere.

The example below fits a two-component mixture to a one-dimensional data set with four points clustered near zero and four near ten. Starting the means deliberately off-center, at $0$ and $8$, EM recovers the cluster means at $1$ and $10$ with equal mixing weights in a couple of iterations, and the printed log-likelihood history is monotonically nondecreasing exactly as the theory promises.
...
```{python}
from aiinaction.ch130_gmm_em import GMMParams, fit_gmm, e_step
import numpy as np

X = [[0.0], [1.0], [2.0], [1.0], [9.0], [10.0], [11.0], [10.0]]
...
print("converged:", result.converged, "in", result.n_iter, "iterations")
print("means:", result.params.means.ravel())
print("weights:", result.params.weights)
print("final log-likelihood:", round(result.log_likelihood, 6))
print("log-likelihood history:", [round(v, 4) for v in result.history])

# Soft responsibilities for the first point (near 0): almost all on component 0.
gamma = e_step(X, init)
print("responsibility row 0:", gamma[0])
```
...
X = reshape(Float64[0, 1, 2, 1, 9, 10, 11, 10], 8, 1)

init = GMMParams(
    [0.5, 0.5],
    reshape(Float64[0.0, 8.0], 2, 1),
    [reshape([1.0], 1, 1), reshape([1.0], 1, 1)],
)

result = fit_gmm(X, init; max_iter=200, tol=1e-10, reg_covar=0.0)

println("converged: ", result.converged, " in ", result.n_iter, " iterations")
println("means: ", result.params.means[:, 1])
println("weights: ", result.params.weights)
println("final log-likelihood: ", round(result.log_likelihood; digits=6))
```

## Rust

```rust
use aiinaction::ch130_gmm_em::{fit_gmm, GmmParams};

fn main() {
    let x = vec![
        vec![0.0], vec![1.0], vec![2.0], vec![1.0],
        vec![9.0], vec![10.0], vec![11.0], vec![10.0],
    ];
    ...
    let result = fit_gmm(&x, &init, 200, 1e-10, 0.0).unwrap();

    println!("converged: {} in {} iterations", result.converged, result.n_iter);
    println!("means: {:?}", result.params.means);
    println!("weights: {:?}", result.params.weights);
    println!("final log-likelihood: {:.6}", result.log_likelihood);
}
```

:::

All three implementations agree to within $10^{-9}$ on a shared set of fixtures (the responsibilities, the initial log-likelihood of $-24.8967$, and the converged log-likelihood of $-14.1241$), and the cross-language parity tests in continuous integration enforce that agreement on every commit. Install the package for your language of choice and the snippets above run as written.
